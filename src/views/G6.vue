  ...
  methods: {
    ...
    initGraph() {
      ...
      const mapNodeSizeAndFontSize = (nodes, propertyName, visualRange) => {
        ...
        let maxp = -9999999999;
        nodes.forEach((node) => {
          const propertyValue = node[propertyName] > 50000 ? 50000 : node[propertyName];
        ...
        nodes.forEach((node) => {
          ...
          if (node.labelCfg && node.labelCfg.style) {
            ...
            let fontSize = (0.45 * node.size) / textLength;
      ...
      const Colors = {noun: '#A41B29', adj: '#f2e394', v: '#d96459', adv: '#f2ae72'};
